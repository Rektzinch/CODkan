...
export default function Home(){
  ...
  return <main className="app">
    ...
    <nav className="bottom"><button className={tab==="home"?"active":""} onClick={()=>{clear();setTab("home")}}><Icon name="search"/><span>Beranda</span></button><button className={tab==="sell"?"active":""} onClick={()=>{clear();setTab("sell")}}><Icon name="plus"/><span>Jual</span></button><button className={tab==="activity"?"active":""} onClick={()=>{clear();setTab("activity");void openNotifications()}}><Icon name="bell"/>{unreadNotifications>0&&<i>{unreadNotifications>9?"9+":unreadNotifications}</i>}<span>Aktivitas</span></button><button className={tab==="profile"?"active":""} onClick={()=>{clear();setTab("profile")}}><Icon name="user"/><span>Profil</span></button></nav>
  </main>
}
