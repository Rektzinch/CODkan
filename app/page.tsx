...
type Profile = { id:string; display_name:string; phone:string|null; area:string; avatar_url:string|null; completed_deals:number; no_shows:number; created_at:string; onboarded:boolean; notifications_enabled:boolean; deletion_requested_at:string|null };
...
export default function Home(){
  ...
  async function requestAccountDeletion(){if(!user||!profile||profile.deletion_requested_at)return;if(!window.confirm("Kirim permintaan penghapusan akun? Deal aktif harus diselesaikan terlebih dahulu."))return;clear();setSubmitting(true);const {error:err}=await supabase.from("profiles").update({deletion_requested_at:new Date().toISOString()}).eq("id",user.id);setSubmitting(false);if(err){fail(err.message);return}setProfile({...profile,deletion_requested_at:new Date().toISOString()});setMessage("Permintaan penghapusan akun telah dicatat.")}
