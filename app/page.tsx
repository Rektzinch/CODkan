...
export default function Home(){
  ...
  const [listings,setListings]=useState<Listing[]>([]); const [offers,setOffers]=useState<Offer[]>([]); const [myOffers,setMyOffers]=useState<Offer[]>([]); const [deals,setDeals]=useState<Deal[]>([]); const [contacts,setContacts]=useState<Contact[]>([]);
  ...
  const loadData=useCallback(async(currentUser:User,currentProfile:Profile)=>{
    ...
    const [listingResult,offerResult,myOfferResult,dealResult,contactResult]=await Promise.all([
  ...
  return <main className="app">
    <header><div className="header-inner"><button className="brand plain" onClick={()=>setTab("home")}>COD<span>kan</span></button><button className="area"><Icon name="pin"/><span><small>Area aktif</small><strong>{profile.area}</strong></span></button><div className="header-search"><Icon name="search"/><input aria-label="Cari barang" placeholder="Cari barang di sekitar…" value={query} onChange={e=>setQuery(e.target.value)}/></div><nav><button onClick={()=>setTab("sell")}><Icon name="plus"/>Jual Barang</button><button onClick={()=>setTab("activity")}><Icon name="bell"/>{offers.length+myOffers.filter(o=>o.source==="seller_counter"&&o.status==="pending").length>0&&<i/>}</button><button onClick={()=>setTab("profile")} className="avatar">{profile.display_name.slice(0,2).toUpperCase()}</button></nav></div></header>
...
function DealRoom({deal,userId,busy,contact,onPropose,onAccept,onConfirm,onCancel}:{deal:Deal;userId:string;busy:boolean;contact?:Contact;onPropose:(e:FormEvent<HTMLFormElement>,id:string)=>void;onAccept:(id:string)=>void;onConfirm:(id:string)=>void;onCancel:(id:string)=>void}){
  ...
  const showForm=awaitingSchedule||rescheduling;
  const scheduleForm=<form className="schedule-form" onSubmit={e=>{setRescheduling(false);onPropose(e,deal.id)}}><h4>{awaitingSchedule?"Usulkan jadwal COD":"Usulkan waktu lain"}</h4><div className="date-time-fields"><label>Tanggal WITA<input name="scheduled_date" inputMode="numeric" placeholder="DDMMYYYY" pattern="\d{8}" maxLength={8} onInput={event=>event.currentTarget.value=event.currentTarget.value.replace(/\D/g,"").slice(0,8)} required/><small>Contoh: 08092026</small></label><label>Waktu<input name="scheduled_time" inputMode="numeric" placeholder="HHMM" pattern="\d{4}" maxLength={4} onInput={event=>event.currentTarget.value=event.currentTarget.value.replace(/\D/g,"").slice(0,4)} required/><small>Contoh: 1630</small></label></div><input name="location_name" placeholder="Nama tempat umum" minLength={3} maxLength={160} required/><input name="location_address" placeholder="Alamat titik temu" minLength={3} maxLength={300} required/><textarea name="notes" placeholder="Catatan pertemuan (opsional)" maxLength={500}/><div className="schedule-actions">{!awaitingSchedule&&<button type="button" disabled={busy} onClick={()=>setRescheduling(false)}>Batal</button>}<button className="primary" disabled={busy}>Kirim usulan</button></div></form>;
  return <article className="deal"><div className="deal-top"><div><small>{deal.status.replaceAll("_"," ").toUpperCase()}</small><h3>{deal.listing?.title||"Barang dalam transaksi"}</h3><p>COD dengan {other.display_name}</p></div><strong>{rupiah(deal.final_price)}</strong></div>
    ...
    :<div className="schedule"><p><strong>{dateTime(schedule.scheduled_at)}</strong><span>{schedule.location_name}</span><small>{schedule.location_address}</small></p>{schedule.status==="proposed"&&schedule.proposed_by!==userId&&<div className="schedule-actions"><button disabled={busy} onClick={()=>setRescheduling(true)}>Usulkan waktu lain</button><button className="primary" disabled={busy} onClick={()=>onAccept(deal.id)}>Setujui jadwal</button></div>}{schedule.status==="proposed"&&schedule.proposed_by===userId&&<em>Menunggu persetujuan pihak lain</em>}{schedule.status==="accepted"&&open&&<button disabled={busy} onClick={()=>setRescheduling(true)}>Ubah jadwal</button>}</div>}
