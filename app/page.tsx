...
export default function Home(){
  ...
  return <main className="app">
    <header><div className="header-inner"><button className="brand plain" aria-label="Beranda CODkan" onClick={()=>{clear();setTab("home")}}><img src="/brand/codkan-lockup.png" alt="CODkan — Ketemu, Tawar, Deal, COD"/></button><button className="area"><Icon name="pin"/><span><small>Area aktif</small><strong>{profile.area}</strong></span></button><div className="header-search"><Icon name="search"/><input aria-label="Cari barang" placeholder="Cari barang di sekitar…" value={query} onChange={e=>setQuery(e.target.value)}/></div><nav><button onClick={()=>{clear();setTab("sell")}}><Icon name="plus"/>Jual Barang</button><button aria-label="Notifikasi" onClick={()=>void openNotifications()}><Icon name="bell"/>{unreadNotifications>0&&<i>{unreadNotifications>9?"9+":unreadNotifications}</i>}</button><button onClick={()=>{clear();setTab("profile")}} className="avatar">{profile.display_name.slice(0,2).toUpperCase()}</button></nav><button className="header-notification" aria-label="Buka notifikasi" onClick={()=>void openNotifications()}><Icon name="bell"/>{unreadNotifications>0&&<i>{unreadNotifications>9?"9+":unreadNotifications}</i>}</button></div></header>
