...
type Profile = { id:string; display_name:string; phone:string|null; area:string; avatar_url:string|null; completed_deals:number; no_shows:number; created_at:string; onboarded:boolean; notifications_enabled:boolean };
type PublicProfile = Omit<Profile,"phone">;
type Media = { id:string; public_url:string; sort_order:number; storage_path?:string };
...
type NotificationTone = "accent"|"success"|"warning"|"neutral";
const notificationMeta:Record<string,{title:string;icon:string;tone:NotificationTone}> = {
  "offer.created":{title:"Tawaran baru masuk",icon:"handshake",tone:"accent"},
  "offer.rejected":{title:"Tawaran belum diterima",icon:"close",tone:"warning"},
  "offer.countered":{title:"Ada harga balasan",icon:"handshake",tone:"accent"},
  "deal.created":{title:"Deal Room siap",icon:"check",tone:"success"},
  "cod.schedule.proposed":{title:"Jadwal COD diusulkan",icon:"pin",tone:"accent"},
  "cod.schedule.countered":{title:"Ada usulan waktu lain",icon:"pin",tone:"warning"},
  "cod.schedule.accepted":{title:"Jadwal COD disepakati",icon:"check",tone:"success"},
  "cod.schedule.rejected":{title:"Usulan jadwal ditolak",icon:"close",tone:"warning"},
  "deal.cancelled":{title:"Deal dibatalkan",icon:"close",tone:"warning"},
  "deal.confirmation":{title:"Konfirmasi COD diterima",icon:"shield",tone:"success"},
  "deal.completed":{title:"COD selesai",icon:"check",tone:"success"},
};

function notificationDetail(notification:Notification){
  const payload=notification.payload||{};
  const amount=Number(payload.amount||payload.final_price||0);
  const scheduledAt=typeof payload.scheduled_at==="string"?payload.scheduled_at:"";
  const location=typeof payload.location_name==="string"?payload.location_name:"";
  const reason=typeof payload.reason==="string"?payload.reason:"";
  if(amount>0)return `${notification.type==="deal.created"?"Harga deal":"Nominal"} ${rupiah(amount)}. Buka aktivitas untuk melihat detailnya.`;
  if(scheduledAt&&location)return `${dateTime(scheduledAt)} di ${location}.`;
  if(scheduledAt)return `${dateTime(scheduledAt)}. Buka aktivitas untuk melihat lokasi.`;
  if(reason)return `Alasan: ${reason}`;
  if(notification.type.startsWith("offer."))return "Buka aktivitas untuk meninjau tawaran dan langkah berikutnya.";
  if(notification.type.startsWith("cod.schedule."))return "Periksa jadwal dan titik temu di Deal Room.";
  return "Buka aktivitas untuk melihat detail pembaruan ini.";
}

function Icon({name,size=20}:{name:string;size?:number}) {
  const paths:Record<string,React.ReactNode>={home:<><path d="m3 11 9-8 9 8"/><path d="M5 10v10h14V10M9 20v-6h6v6"/></>,search:<><circle cx="11" cy="11" r="7"/><path d="m20 20-4-4"/></>,pin:<><path d="M20 10c0 5-8 11-8 11S4 15 4 10a8 8 0 1 1 16 0Z"/><circle cx="12" cy="10" r="2.5"/></>,plus:<><circle cx="12" cy="12" r="9"/><path d="M8 12h8M12 8v8"/></>,bell:<><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9"/><path d="M10 21h4"/></>,user:<><circle cx="12" cy="8" r="4"/><path d="M4 21a8 8 0 0 1 16 0"/></>,mail:<><rect x="3" y="5" width="18" height="14" rx="2"/><path d="m3 7 9 6 9-6"/></>,phone:<><rect x="6" y="3" width="12" height="18" rx="2"/><path d="M10 18h4"/></>,lock:<><rect x="5" y="10" width="14" height="11" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></>,eye:<><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.5"/></>,eyeoff:<><path d="m3 3 18 18"/><path d="M10.6 6.2A10.5 10.5 0 0 1 12 6c6 0 9.5 6 9.5 6a17.4 17.4 0 0 1-3 3.7M6.3 6.3A17.6 17.6 0 0 0 2.5 12S6 18 12 18c1.4 0 2.6-.3 3.7-.8"/><path d="M9.9 9.9a3 3 0 0 0 4.2 4.2"/></>,close:<><path d="m6 6 12 12M18 6 6 18"/></>,shield:<><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10Z"/><path d="m9 12 2 2 4-4"/></>,handshake:<><path d="m8 13-2 2a2.4 2.4 0 0 1-3.4-3.4l4.1-4.1a3 3 0 0 1 3.5-.55l1.1.55 1.8-1.8a3 3 0 0 1 4.25 0l4.05 4.05a2.4 2.4 0 0 1-3.4 3.4l-2-2"/><path d="m8 13 4.2 4.2a2.15 2.15 0 0 0 3.04-3.04l-.15-.15a2.15 2.15 0 0 0 3.04-3.04l-2.9-2.9-2.3 2.3a2.2 2.2 0 0 1-3.1 0L8.2 8.75"/></>,box:<><path d="m3 7 9-4 9 4-9 4-9-4Z"/><path d="M3 7v10l9 4 9-4V7M12 11v10"/></>,check:<path d="m5 12 4 4L19 6"/>,logout:<><path d="M10 17l5-5-5-5M15 12H3"/><path d="M14 3h7v18h-7"/></>,down:<path d="m7 10 5 5 5-5"/>,upload:<><path d="M12 16V4m0 0L7 9m5-5 5 5"/><path d="M5 15v5h14v-5"/></>,google:<><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-1 .67-2.26 1.07-3.71 1.07-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.11c-.22-.66-.35-1.36-.35-2.11s.13-1.45.35-2.11V7.05H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.95l3.66-2.84z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.66l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.05l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></>,facebook:<path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z" fill="#1877F2"/>,apple:<path d="M12 20.94c-1.67 0-3.13-.88-4.08-2.2-1.25-1.74-1.25-4.58 0-6.32 1.52-2.13 4.7-2.12 6.22 0 1.25 1.74 1.25 4.58 0 6.32-.95 1.32-2.41 2.2-4.14 2.2zM12 2c.28 0 .5.22.5.5v2c0 .28-.22.5-.5.5s-.5-.22-.5-.5v-2c0-.28.22-.5.5-.5z" fill="currentColor"/>};
  return <svg aria-hidden="true" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">{paths[name]}</svg>;
}

function NotificationCard({notification,onOpen}:{notification:Notification;onOpen:()=>void}){
  const meta=notificationMeta[notification.type]||{title:"Pembaruan CODkan",icon:"bell",tone:"neutral" as NotificationTone};
  return <article className={`notification-card ${notification.read_at?"":"unread"}`} data-tone={meta.tone}>
    <button type="button" onClick={onOpen} aria-label={`${notification.read_at?"":"Belum dibaca. "}${meta.title}. ${notificationDetail(notification)}`}>
      <span className="notification-card__icon"><Icon name={meta.icon} size={19}/></span>
      <span className="notification-card__copy">
        <span className="notification-card__heading"><strong>{meta.title}</strong>{!notification.read_at&&<i className="notification-card__dot" aria-hidden="true"/>}</span>
        <span className="notification-card__detail">{notificationDetail(notification)}</span>
        <time dateTime={notification.created_at}>{dateTime(notification.created_at)}</time>
      </span>
    </button>
  </article>;
}
...
function AuthScreen({busy,error,notice,onSubmit,onClear}:{busy:boolean;error:string;notice:string;onSubmit:(mode:"signin"|"signup",email:string,password:string,name?:string,phone?:string)=>void;onClear:()=>void}){
  ...
  return <main className={`auth-page auth-page--${mode}`}>
    ...
    {mode==="signin"&&<div className="auth-scene-mobile" aria-hidden="true"><img src="/brand/cod-login-scene.jpg" alt=""/></div>}
  </main>;
}
...
export default function Home(){
  ...
  const [listings,setListings]=useState<Listing[]>([]); const [myListings,setMyListings]=useState<Listing[]>([]); const [editingListingId,setEditingListingId]=useState<string|null>(null); const [offers,setOffers]=useState<Offer[]>([]); const [myOffers,setMyOffers]=useState<Offer[]>([]); const [deals,setDeals]=useState<Deal[]>([]); const [contacts,setContacts]=useState<Contact[]>([]); const [notifications,setNotifications]=useState<Notification[]>([]);
  const [tab,setTab]=useState<"home"|"sell"|"activity"|"listings"|"profile">("home"); const [query,setQuery]=useState(""); const [category,setCategory]=useState("Semua"); const [notificationOpen,setNotificationOpen]=useState(false);
  ...
  const loadData=useCallback(async(currentUser:User,currentProfile:Profile)=>{
    ...
    const [listingResult,myListingResult,offerResult,myOfferResult,dealResult,contactResult,notificationResult]=await Promise.all([
      ...
      supabase.from("listings").select("*,listing_media(*)").eq("seller_id",currentUser.id).order("created_at",{ascending:false}),
    ...
    if(!myListingResult.error) setMyListings((myListingResult.data||[]) as unknown as Listing[]);
  ...
  useEffect(()=>{
    ...
    const closeOnEscape=(event:KeyboardEvent)=>{if(event.key==="Escape")setNotificationOpen(false)};
  ...
  const activeDeals=useMemo(()=>deals.filter(deal=>deal.status!=="completed"&&deal.status!=="cancelled"),[deals]);
  ...
  async function uploadAvatar(event:ChangeEvent<HTMLInputElement>){const file=event.target.files?.[0];event.target.value="";if(!user||!profile||!file)return;if(!file.type.startsWith("image/")){fail("Pilih file gambar JPG, PNG, atau WebP.");return}if(file.size>MAX_PHOTO_BYTES){fail("Ukuran foto profil maksimal 5 MB.");return}clear();setSubmitting(true);const extension=file.name.split(".").pop()?.toLowerCase().replace(/[^a-z0-9]/g,"")||"jpg";const path=`avatars/${user.id}/${Date.now()}.${extension}`;const uploaded=await supabase.storage.from("listing-media").upload(path,file,{contentType:file.type,upsert:false});if(uploaded.error){setSubmitting(false);fail(uploaded.error.message);return}const publicUrl=supabase.storage.from("listing-media").getPublicUrl(path).data.publicUrl;const {error:profileError}=await supabase.from("profiles").update({avatar_url:publicUrl}).eq("id",user.id);if(profileError){await supabase.storage.from("listing-media").remove([path]);setSubmitting(false);fail(profileError.message);return}setProfile({...profile,avatar_url:publicUrl});setSubmitting(false);setMessage("Foto profil berhasil diperbarui.")}
  async function setNotificationsEnabled(enabled:boolean){if(!user||!profile)return;clear();setSubmitting(true);const {error:err}=await supabase.from("profiles").update({notifications_enabled:enabled}).eq("id",user.id);setSubmitting(false);if(err){fail(err.message);return}setProfile({...profile,notifications_enabled:enabled});setMessage(enabled?"Notifikasi realtime diaktifkan.":"Notifikasi realtime dinonaktifkan.")}
  async function changePassword(e:FormEvent<HTMLFormElement>){e.preventDefault();clear();const form=new FormData(e.currentTarget);const password=String(form.get("new_password"));if(password!==String(form.get("confirmation"))){fail("Konfirmasi kata sandi belum sama.");return}setSubmitting(true);const {error:err}=await supabase.auth.updateUser({password});setSubmitting(false);if(err){fail(err.message);return}(e.currentTarget).reset();setMessage("Kata sandi berhasil diperbarui.")}
  async function deleteAccount(){if(!user||!profile)return;if(!window.confirm("Hapus akun CODkan sekarang? Semua listing, tawaran, deal, notifikasi, dan foto profil akan dihapus permanen."))return;clear();setSubmitting(true);const {error:err}=await supabase.rpc("delete_my_account");setSubmitting(false);if(err){fail(err.message);return}await supabase.auth.signOut();setUser(null);setProfile(null);setListings([]);setMyListings([]);setOffers([]);setMyOffers([]);setDeals([]);setContacts([]);setNotifications([]);setTab("home");window.alert("Akun CODkan dan seluruh datanya telah dihapus.")}
  async function updateListing(e:FormEvent<HTMLFormElement>,listingId:string){e.preventDefault();if(!user||!profile)return;clear();const form=new FormData(e.currentTarget);const price=Number(String(form.get("price")).replace(/\D/g,""));if(!price){fail("Harga barang harus lebih dari nol.");return}setSubmitting(true);const {error:err}=await supabase.from("listings").update({title:String(form.get("title")).trim(),description:String(form.get("description")).trim(),price,category:String(form.get("category")),condition:String(form.get("condition")),negotiable:form.get("negotiable")==="on",visibility_radius_km:Number(form.get("radius"))}).eq("id",listingId).eq("seller_id",user.id);setSubmitting(false);if(err){fail(err.message);return}setEditingListingId(null);setMessage("Listing diperbarui.");await loadData(user,profile)}
  async function setListingVisibility(listing:Listing){if(!user||!profile||!["active","hidden","draft"].includes(listing.status))return;clear();const nextStatus=listing.status==="active"?"hidden":"active";setSubmitting(true);const {error:err}=await supabase.from("listings").update({status:nextStatus}).eq("id",listing.id).eq("seller_id",user.id);setSubmitting(false);if(err){fail(err.message);return}setMessage(nextStatus==="active"?"Listing ditayangkan.":"Listing dinonaktifkan.");await loadData(user,profile)}
  async function deleteListing(listing:Listing){if(!user||!profile||!["active","hidden","draft","sold"].includes(listing.status))return;if(!window.confirm(`Hapus \"${listing.title}\"? Listing dan fotonya tidak dapat dipulihkan.`))return;clear();setSubmitting(true);const paths=listing.listing_media.map(media=>media.storage_path).filter((path):path is string=>Boolean(path));if(paths.length){const {error:storageError}=await supabase.storage.from("listing-media").remove(paths);if(storageError){setSubmitting(false);fail(storageError.message);return}}const {error:err}=await supabase.from("listings").delete().eq("id",listing.id).eq("seller_id",user.id);setSubmitting(false);if(err){fail(err.message);return}setEditingListingId(null);setMessage("Listing dihapus.");await loadData(user,profile)}
  ...
  function openNotifications(){clear();setNotificationOpen(true)}
  async function markNotificationsRead(){if(!unreadNotifications)return;setNotifications(current=>current.map(notification=>notification.read_at?notification:{...notification,read_at:new Date().toISOString()}));const {error:err}=await supabase.rpc("mark_all_notifications_read");if(err){fail(err.message);return}if(user&&profile)await loadData(user,profile)}
  ...
  return <main className="app">
    <header><div className="header-inner"><button className="brand plain" aria-label="Beranda CODkan" onClick={()=>{clear();setTab("home")}}><img src="/brand/codkan-lockup.png" alt="CODkan — Ketemu, Tawar, Deal, COD"/></button><button className="area"><Icon name="pin"/><span><small>Area aktif</small><strong>{profile.area}</strong></span></button><div className="header-search"><Icon name="search"/><input aria-label="Cari barang" placeholder="Cari barang di sekitar…" value={query} onChange={e=>setQuery(e.target.value)}/></div><nav><button onClick={()=>{clear();setTab("sell")}}><Icon name="plus"/>Jual Barang</button><button aria-label="Notifikasi" onClick={()=>void openNotifications()}><Icon name="bell"/>{unreadNotifications>0&&<i>{unreadNotifications>9?"9+":unreadNotifications}</i>}</button><button onClick={()=>{clear();setTab("profile")}} className="avatar" aria-label="Buka profil">{profile.avatar_url?<img src={profile.avatar_url} alt=""/>:<span>{profile.display_name.slice(0,2).toUpperCase()}</span>}</button></nav><button className="header-notification" aria-label="Buka notifikasi" onClick={()=>void openNotifications()}><Icon name="bell"/>{unreadNotifications>0&&<i>{unreadNotifications>9?"9+":unreadNotifications}</i>}</button></div></header>
    {notificationOpen&&<div className="notification-scrim" onMouseDown={event=>{if(event.target===event.currentTarget){setNotificationOpen(false);void markNotificationsRead()}}}><section className="notification-panel" role="dialog" aria-modal="true" aria-labelledby="notification-title"><div className="notification-title"><div><h2 id="notification-title">Notifikasi</h2><p>Tawaran, deal, dan jadwal COD terbaru.</p></div><button type="button" aria-label="Tutup notifikasi" onClick={()=>{setNotificationOpen(false);void markNotificationsRead()}}><Icon name="close" size={17}/></button></div>{notifications.length===0?<div className="notification-empty"><Icon name="bell" size={28}/><strong>Belum ada notifikasi</strong><span>Pembaruan tawaran, Deal Room, dan jadwal COD akan muncul di sini.</span></div>:<div className="notification-list">{notifications.map(notification=><NotificationCard key={notification.id} notification={notification} onOpen={()=>{clear();setTab("activity");setNotificationOpen(false);void markNotificationsRead()}}/>)}</div>}</section></div>}
    ...
    {tab==="home"&&<section className="page home-page"><div className="mobile-title"><p><Icon name="pin" size={15}/> {profile.area}</p><h1>Barang nyata<br/><em>dekat kamu.</em></h1></div><div className="mobile-search"><Icon name="search"/><input aria-label="Cari barang di CODkan" placeholder="Cari barang di CODkan…" value={query} onChange={e=>setQuery(e.target.value)}/></div><div className="chips"><button className={category==="Semua"?"active":""} onClick={()=>setCategory("Semua")}>Semua</button>{categories.map(c=><button className={category===c?"active":""} onClick={()=>setCategory(c)} key={c}>{c}</button>)}</div><div className="heading"><div><h1>Barang dekat kamu</h1><p className="heading-support">Pilihan terbaru di {profile.area}</p></div><button className="view-all" onClick={()=>setCategory("Semua")}>Lihat semua <span>›</span></button></div>{visible.length===0?<div className="empty"><img src="/brand/sticker-siap-cod.jpg" alt="Si COD siap COD"/><h2>Belum ada barang di area ini</h2><p>Jadilah yang pertama memasang barang untuk pembeli di {profile.area}.</p><button className="primary" onClick={()=>setTab("sell")}>Pasang barang pertama</button></div>:<div className="grid">{visible.map(item=><article key={item.id} role="button" tabIndex={0} aria-label={`Lihat ${item.title}, ${rupiah(item.price)}`} onClick={()=>setSelected(item)} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();setSelected(item)}}}><div className="photo">{item.listing_media[0]?<img src={item.listing_media[0].public_url} alt={item.title}/>:<span>Foto tidak tersedia</span>}{item.negotiable&&<b>Bisa ditawar</b>}</div><div className="card-copy"><strong>{rupiah(item.price)}</strong><h2>{item.title}</h2><p><Icon name="pin" size={14}/>{item.area}</p><small>{item.condition}</small></div></article>)}</div>}</section>}
    {tab==="sell"&&<section className="page narrow"><div className="heading heading-with-sticker"><div><h1>Pasang barang baru</h1><p className="heading-support">Tulis detail yang jelas agar pembeli cepat yakin.</p></div><img src="/brand/sticker-siap-cod.jpg" alt="Si COD siap membantu"/></div><form className="panel form" onSubmit={createListing}><FilePicker/><label>Judul<input name="title" minLength={5} maxLength={120} required/></label><label>Deskripsi<textarea name="description" minLength={10} maxLength={4000} rows={5} required/></label><div className="two"><label>Harga<CurrencyInput name="price"/></label><label>Radius penemuan<CustomSelect name="radius" defaultValue="10" options={[{value:"5",label:"5 km"},{value:"10",label:"10 km"},{value:"25",label:"25 km"},{value:"50",label:"50 km"}]}/></label></div><div className="two"><label>Kategori<CustomSelect name="category" defaultValue={categories[0]} options={categories.map(value=>({value,label:value}))}/></label><label>Kondisi<CustomSelect name="condition" defaultValue={conditions[0]} options={conditions.map(value=>({value,label:value}))}/></label></div><ToggleField name="negotiable" label="Harga bisa ditawar maksimal tiga kali"/><div className="safety"><Icon name="shield"/><span><strong>Pembayaran wajib COD</strong><small>Kontak dan lokasi presisi tidak ditampilkan sebelum deal.</small></span></div><button className="primary" disabled={submitting}>{submitting?"Memasang…":"Pasang barang"}</button></form></section>}
    {tab==="activity"&&<section className="page"><div className="heading heading-with-sticker"><div><h1>Aktivitas transaksi</h1><p className="heading-support">Kelola tawaran, jadwal, dan Deal Room dalam satu tempat.</p></div><img src="/brand/sticker-deal.jpg" alt="Si COD menyambut deal"/></div><div className="activity-layout"><section className="activity-area activity-area--incoming"><h2>Tawaran masuk <span>{offers.length}</span></h2>{offers.length===0?<div className="mini-empty">Belum ada tawaran yang menunggu jawaban.</div>:offers.map(o=><IncomingOffer key={o.id} offer={o} busy={submitting} onRespond={respond}/>)}</section><section className="activity-area activity-area--mine"><h2>Tawaranmu <span>{myOffers.length}</span></h2>{myOffers.length===0?<div className="mini-empty">Tawaran yang kamu kirim akan muncul di sini.</div>:myOffers.map(o=><MyOffer key={o.id} offer={o} busy={submitting} onRespond={respond}/>)}</section><section className="activity-area activity-area--active"><h2>Deal aktif <span>{activeDeals.length}</span></h2>{activeDeals.length===0?<div className="mini-empty">Deal Room dibuat otomatis setelah tawaran diterima.</div>:activeDeals.map(d=><DealRoom key={d.id} deal={d} userId={user.id} busy={submitting} contact={contacts.find(c=>c.deal_id===d.id)} onPropose={proposeSchedule} onAccept={acceptSchedule} onReject={rejectSchedule} onConfirm={confirmDeal} onCancel={cancelDeal} onDismiss={dismissDealHistory}/>)}</section><section className="activity-area activity-area--history"><h2>Riwayat deal <span>{visibleHistoryDeals.length}</span></h2>{visibleHistoryDeals.length===0?<div className="mini-empty">Belum ada riwayat deal selesai atau batal.</div>:visibleHistoryDeals.map(d=><DealRoom key={d.id} deal={d} userId={user.id} busy={submitting} contact={contacts.find(c=>c.deal_id===d.id)} onPropose={proposeSchedule} onAccept={acceptSchedule} onReject={rejectSchedule} onConfirm={confirmDeal} onCancel={cancelDeal} onDismiss={dismissDealHistory}/>)}</section></div></section>}
    {tab==="profile"&&<section className="page profile-page"><div className="heading"><div><h1>{profile.display_name}</h1><p className="heading-support">Kelola identitas dan preferensi akunmu.</p></div></div><section className="profile-overview"><div className="profile-avatar">{profile.display_name.slice(0,2).toUpperCase()}</div><div><strong>{profile.area}</strong><p>{profile.completed_deals} COD selesai · {profile.no_shows} no-show</p><small>Bergabung {new Intl.DateTimeFormat("id-ID",{month:"long",year:"numeric"}).format(new Date(profile.created_at))}</small></div><button className="profile-signout" onClick={signOut} disabled={submitting}><Icon name="logout" size={17}/>Keluar</button></section><section className="profile-section profile-photo-card"><div className="profile-photo-preview">{profile.avatar_url?<img src={profile.avatar_url} alt={`Foto profil ${profile.display_name}`}/>:<span>{profile.display_name.slice(0,2).toUpperCase()}</span>}</div><div><b>Foto profil</b><small>Gunakan foto yang jelas agar mudah dikenali saat bertransaksi.</small><label className="profile-upload-button">{submitting?"Mengunggah…":"Pilih foto"}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={uploadAvatar} disabled={submitting}/></label></div></section><details className="profile-section" open><summary><span><b>Data akun</b><small>Nama, nomor HP, dan area domisili</small></span><Icon name="down" size={18}/></summary><form className="profile-form" onSubmit={saveProfile}><label>Nama tampilan<input name="name" defaultValue={profile.display_name} minLength={2} maxLength={60} required/></label><label>Nomor telepon <small>Hanya dibuka setelah deal disepakati.</small><input name="phone" type="tel" defaultValue={profile.phone||""} maxLength={24}/></label><label>Area domisili<input name="area" defaultValue={profile.area} minLength={2} maxLength={120} required/></label><button className="primary" disabled={submitting}>{submitting?"Menyimpan…":"Simpan perubahan"}</button></form></details></section>}{tab==="listings"&&<section className="page listings-page"><div className="heading"><div><h1>Listing saya</h1><p className="heading-support">Pantau status dan kelola barang yang kamu jual.</p></div><button className="primary listing-create" onClick={()=>setTab("sell")}>Pasang barang</button></div><section className="profile-section listing-section"><div className="profile-section-title"><div><b>Listing saya</b><small>Kelola barang berdasarkan statusnya.</small></div><span>{myListings.length}</span></div>{myListings.length===0?<div className="profile-empty"><Icon name="box" size={24}/><p>Belum ada listing. Pasang barang pertamamu untuk mulai berjualan.</p><button className="primary" onClick={()=>setTab("sell")}>Pasang barang</button></div>:<div className="my-listings">{myListings.map(item=>{const canManage=["active","hidden","draft","sold"].includes(item.status);const statusLabel=item.status==="sold"?"Terjual":item.status==="reserved"?"Dipesan":item.status==="active"?"Aktif":item.status==="hidden"?"Nonaktif":item.status==="draft"?"Draft":item.status;return <article className="my-listing" key={item.id}><div className="my-listing-media">{item.listing_media[0]?<img src={item.listing_media[0].public_url} alt={item.title}/>:<Icon name="box" size={25}/>}</div><div className="my-listing-copy"><div><span className={`listing-status ${item.status}`}>{statusLabel}</span><strong>{item.title}</strong><p>{rupiah(item.price)} · {item.category}</p></div>{editingListingId===item.id?<form className="listing-edit-form" onSubmit={event=>updateListing(event,item.id)}><label>Judul<input name="title" defaultValue={item.title} minLength={5} maxLength={120} required/></label><label>Deskripsi<textarea name="description" defaultValue={item.description} minLength={10} maxLength={4000} rows={3} required/></label><div className="two"><label>Harga<input name="price" defaultValue={String(item.price)} inputMode="numeric" required/></label><label>Radius<select name="radius" defaultValue={String(item.visibility_radius_km)}>{[5,10,25,50].map(value=><option key={value} value={value}>{value} km</option>)}</select></label></div><div className="two"><label>Kategori<select name="category" defaultValue={item.category}>{categories.map(value=><option key={value} value={value}>{value}</option>)}</select></label><label>Kondisi<select name="condition" defaultValue={item.condition}>{conditions.map(value=><option key={value} value={value}>{value}</option>)}</select></label></div><label className="listing-negotiable"><input name="negotiable" type="checkbox" defaultChecked={item.negotiable}/><span>Harga bisa ditawar</span></label><div className="listing-actions"><button className="primary" disabled={submitting}>Simpan listing</button><button type="button" onClick={()=>setEditingListingId(null)}>Batal</button></div></form>:<div className="listing-actions">{canManage&&<button type="button" onClick={()=>setEditingListingId(item.id)}>Edit</button>}{canManage&&<button type="button" onClick={()=>void setListingVisibility(item)} disabled={submitting}>{item.status==="active"?"Nonaktifkan":"Tayangkan"}</button>}{canManage&&<button type="button" className="danger-button" onClick={()=>void deleteListing(item)} disabled={submitting}>Hapus</button>}{!canManage&&<small>Listing ini terkunci karena sedang masuk proses deal.</small>}</div>}</div></article>})}</div>}</section></section>}{tab==="profile"&&<section className="page profile-page profile-page-continuation"><section className="profile-section profile-settings"><div className="profile-section-title"><div><b>Notifikasi</b><small>Atur pembaruan tawaran, Deal Room, dan jadwal COD.</small></div></div><label className="profile-toggle"><span><strong>Notifikasi realtime</strong><small>Badge dan daftar pembaruan di aplikasi akan diperbarui saat ada aktivitas baru.</small></span><input type="checkbox" checked={profile.notifications_enabled} disabled={submitting} onChange={event=>void setNotificationsEnabled(event.target.checked)}/><i/></label></section><details className="profile-section"><summary><span><b>Keamanan akun</b><small>Perbarui kata sandi akunmu.</small></span><Icon name="down" size={18}/></summary><form className="profile-form security-form" onSubmit={changePassword}><label>Kata sandi baru<input name="new_password" type="password" minLength={8} autoComplete="new-password" required/></label><label>Konfirmasi kata sandi<input name="confirmation" type="password" minLength={8} autoComplete="new-password" required/></label><button className="primary" disabled={submitting}>{submitting?"Memperbarui…":"Ubah kata sandi"}</button></form></details><details className="profile-section"><summary><span><b>Bantuan COD aman</b><small>Ringkasan langkah aman saat bertemu dan bertransaksi.</small></span><Icon name="down" size={18}/></summary><div className="cod-help"><p><strong>1. Pilih tempat umum.</strong> Temui lawan transaksi di lokasi ramai dan terang.</p><p><strong>2. Periksa barang sebelum bayar.</strong> Pastikan kondisi, kelengkapan, dan fungsi barang sesuai kesepakatan.</p><p><strong>3. Jangan membagikan kode rahasia.</strong> Pembayaran dilakukan saat bertemu; jangan lanjutkan permintaan transfer atau OTP di luar kesepakatan COD.</p></div></details><section className="profile-section account-danger"><div><b>Hapus akun</b><small>Penghapusan bersifat permanen dan langsung menghapus data akun, listing, tawaran, deal, serta foto profil.</small></div><button type="button" className="danger-button" disabled={submitting} onClick={()=>void deleteAccount()}>{submitting?"Menghapus…":"Hapus akun sekarang"}</button></section></section>}
    ...
    <nav className="bottom" aria-label="Navigasi utama"><button className={tab==="home"?"active":""} onClick={()=>{clear();setTab("home")}}><Icon name="home"/><span>Beranda</span></button><button onClick={()=>{clear();setTab("home");requestAnimationFrame(()=>document.querySelector<HTMLInputElement>(".mobile-search input")?.focus())}}><Icon name="search"/><span>Cari</span></button><button className={tab==="activity"?"active":""} onClick={()=>{clear();setTab("activity")}}><Icon name="bell"/>{unreadNotifications>0&&<i>{unreadNotifications>9?"9+":unreadNotifications}</i>}<span>Aktivitas</span></button><button className={tab==="listings"?"active":""} onClick={()=>{clear();setTab("listings")}}><Icon name="box"/><span>Listing</span></button><button className={tab==="profile"?"active":""} onClick={()=>{clear();setTab("profile")}}><Icon name="user"/><span>Akun</span></button></nav>
  </main>
}
